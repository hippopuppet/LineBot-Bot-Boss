			...
			case <- checkBossTimer:
				...
				for _, bossinfo := range dbResult[0].BossInfo {
					...
					if JetLag == 0 || JetLag == 7 {
						...
						for _, groupinfo := range dbResult[0].GroupInfo {
							if groupinfo.Active == 1 {
								if _, err := bot.PushMessage(groupinfo.Id, linebot.NewTextMessage("BOSS : "+bossinfo.KingOfName +"Â∞áÂú®"+bossinfo.Resurrection+"?çÁ?! Map: "+ bossinfo.Map)).Do(); err != nil {
									log.Print(err)
								}
							}
						}
					...
					if JetLag == 10 {
						for _, groupinfo := range dbResult[0].GroupInfo {
							if groupinfo.Active == 1 {
								if _, err := bot.PushMessage(groupinfo.Id, linebot.NewTextMessage("BOSS : "+bossinfo.KingOfName +" Â∑≤Á??çÁ?‰∫?!! Map: "+ bossinfo.Map)).Do(); err != nil {
									log.Print(err)
								}
							}
						}
						...
						if len(result) > 1 {
							...
							for _, airinfo := range airJson {
								if airinfo.SiteName == result[1] {
									...
									airinfo_buf.WriteString("??PM2.5 ?Ë©®¬ÄÊΩõÓæ≠ ")
							...
							if isFound == false {
								var airinfo_buf bytes.Buffer
								airinfo_buf.WriteString("?ùÓ??")
								airinfo_buf.WriteString(result[1])
								airinfo_buf.WriteString("??PM2.5 ?àÔã™?")
								if _, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage(airinfo_buf.String())).Do(); err != nil {
										log.Print(err)
								}
							}
						} else {
							if _, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage("?¢Ô???‰∫§ÁÇ¨?‰∫•Â≤∑PM2.5?àÔã™???íÓØµ??)).Do(); err != nil {
									log.Print(err)
							}
						}
					}//!PM
